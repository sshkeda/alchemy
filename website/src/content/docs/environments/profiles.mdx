...
Alchemy creates the default profile in memory and saves it on the first write.
Profiles have stable internal ids, so renaming the default also updates its
credential directory and keeps it selected. Other names must be created first.
For example, `--profile prod` fails until you create `prod`.
...
CLOUDFLARE_ACCOUNT_ID). Alchemy did not use profile 'default'.
```
