...
Profiles are always explicit. If none exist, run `alchemy profile` in an
interactive terminal, or `alchemy profile create <name>` in a script. The first
profile you create becomes the default; creating more profiles does not change
it. Existing installations with a profile named `default` keep it selected
during migration. Profiles have stable internal ids, so renaming the default
also updates its credential directory and keeps it selected.
...
CLOUDFLARE_ACCOUNT_ID) — the stored default profile was not used.
```
